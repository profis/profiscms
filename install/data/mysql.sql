...
CREATE TABLE IF NOT EXISTS `{prefix}pages` (
  ...
  `target` SMALLINT( 1 ) NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`),
  KEY `idp` (`idp`),
  KEY `nomenu` (`nomenu`),
  KEY `deleted` (`deleted`),
...
('form_submitted_text', 'forms', 0, 'en', 'The form “%1$s” on page “%2$s” has been submitted. Below are the contents.'),
('form_submitted_text', 'forms', 0, 'lt', 'Užpildyta tinklapyje „%2$s“ esanti forma „%1$s“. Žemiau – pateiktas jos turinys.'),
('form_submitted_text', 'forms', 0, 'ru', 'На странице «%2$s» заполнена форма «%1$s». Ниже – её содержимое.'),
