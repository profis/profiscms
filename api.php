...
if ($routes->Get(1) == 'admin') {
		...
		case 'info':
			switch (v($routes->Get(2))) {
				case 'php':
					phpinfo();
					break;
				default: echo 'You can view this information: PHP';
			}
			break;
		case 'plugin':
			...
			break;
		default: echo 'Please define API action that you want to execute';
	}
}
else {
	//public API
	...
	switch($routes->Get(1)) {
		case 'texts':
			...
			<center style="font-size:14pt;">
				...
				foreach ($site->Get_languages() as $code=>$ln) {
					$a++;
					if ($a>1) echo ' | ';
					echo '<a href="admin/api/texts/'.$code.'/">'.$ln.'</a>';
				}
				?>
			</center>
			<?php
		...
		case 'page':
		default: echo 'No API action specified';
	}
}