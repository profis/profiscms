	...
	tinymce.create('tinymce.plugins.MediaPlugin', {
		init : function(ed, url) {
			...
			function isMediaElm(n) {
				return /^(mceItemChart|mceItemGmap|mceItemFlash|mceItemShockWave|mceItemWindowsMedia|mceItemQuickTime|mceItemRealMedia)$/.test(n.className);
			};
			...
			ed.addCommand('mceMedia', function() {
				PC.dialog.media.show(
					(ed.parents==undefined?undefined:(ed.parents[0].className=='mceItemFlash'?ed.parents[0]:undefined))
				);
				return;
			...
			ed.addCommand('mceChart', function() {
				mod_datsets_click(Ext.util.JSON.decode('{'+ed.parents[0].title+'}'));
				return;
			});
			...
			ed.onInit.add(function() {
				...
				if (ed && ed.plugins.contextmenu) {
					ed.plugins.contextmenu.onContextMenu.add(function(th, m, e) {
						if (e.nodeName == 'IMG' && /mceItem(Chart|Gmap|Flash|ShockWave|WindowsMedia|QuickTime|RealMedia)/.test(e.className)) {
							if (e.className == 'mceItemChart') {
								m.add({title : 'media.edit', icon : 'media', cmd : 'mceChart'});
							}
							else if (e.className == 'mceItemGmap') {
								m.add({title : 'media.edit_gmap', icon : 'gmap', cmd : 'mceGmap'});
							} else {
				...
				if (o.get) {
					each(dom.select('IMG', o.node), function(n) {
						...
						switch (n.className) {
							case 'mceItemChart':
								ci = 'google-chart';
								cb = 'http://code.google.com/apis/chart/';
								mt = 'application/google-chart';
								break;
								
							case 'mceItemGmap':
								ci = 'google-map';
								cb = 'http://maps.google.com/';
								mt = 'application/google-map';
								break;
			...
			ed.onPostProcess.add(function(ed, o) {
				if (ed.getParam('media_use_script')) {
					o.content = o.content.replace(/<img[^>]+>/g, function(im) {
						...
						if (/^(mceTempChart|mceTempGmap|mceTempFlash|mceTempShockWave|mceTempWindowsMedia|mceTempQuickTime|mceTempRealMedia)$/.test(cl)) {
		...
		_objectsToSpans : function(ed, o) {
			...
			h = h.replace(/<script[^>]*>\s*write(Chart|Gmap|Flash|ShockWave|WindowsMedia|QuickTime|RealMedia)\(\{([^\)]*)\}\);\s*<\/script>/gi, function(a, b, c) {
				var o = t._parse(c);
		...
		_spansToImgs : function(p) {
			...
			each(dom.select('span', p), function(n) {
				...
				if (dom.getAttrib(n, 'class') == 'mceItemObject') {
					...
					switch (ci) {
						case 'clsid:google-chart':
							dom.replace(t._createImg('mceItemChart', n), n);
							break;
							
						case 'clsid:google-map':
							dom.replace(t._createImg('mceItemGmap', n), n);
							break;
				...
				if (dom.getAttrib(n, 'class') == 'mceItemEmbed') {
					switch (dom.getAttrib(n, 'type')) {
						case 'application/google-chart':
							dom.replace(t._createImg('mceItemChart', n), n);
							break;
							
						case 'application/google-map':
							dom.replace(t._createImg('mceItemGmap', n), n);
							break;
