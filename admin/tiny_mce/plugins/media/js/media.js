...
function init() {
	...
	if (/mceItem(Chart|Gmap|Flash|ShockWave|WindowsMedia|QuickTime|RealMedia)/.test(ed.dom.getAttrib(fe, 'class'))) {
		pl = fe.title;
...
function insertMedia() {
	...
	fe = ed.selection.getNode();
	if (fe != null && /mceItem(Chart|Gmap|Flash|ShockWave|WindowsMedia|QuickTime|RealMedia)/.test(ed.dom.getAttrib(fe, 'class'))) {
		switch (f.media_type.options[f.media_type.selectedIndex].value) {
			case "chart":
				fe.className = "mceItemChart";
				break;				
			case "gmap":
				fe.className = "mceItemGmap";
				break;
...
function getType(v) {
	...
	fo = ed.getParam("media_types", "chart=chart;gmap=gmap;flash=swf;flv=flv;shockwave=dcr;qt=mov,qt,mpg,mp3,mp4,mpeg;shockwave=dcr;wmp=avi,wmv,wm,asf,asx,wmx,wvx;rmp=rm,ra,ram").split(';');
...
function changedType(t) {
	...
	d.getElementById('gmap_options').style.display = 'none';
...
function serializeParameters() {
	...
	if (f.media_type.options[f.media_type.selectedIndex].value != 'gmap') {
		s += getStr(null, 'src');
...
function getStr(p, n, d) {
	var elem_name = (p != null ? p + "_" : "") + n;
	var e = document.forms[0].elements[elem_name];
...
function generatePreview(c) {
	...
	switch (f.media_type.options[f.media_type.selectedIndex].value) {		case "flash":			//flash clsid cls = 'clsid:D27CDB6E-AE6D-11cf-96B8-444553540000';			cls = 'clsid:google-chart';			codebase = 'http://code.google.com/apis/chart/';			type = 'application/google-chart';			break;	
		case "flash":
			//flash clsid cls = 'clsid:D27CDB6E-AE6D-11cf-96B8-444553540000';
			cls = 'clsid:google-map';
			codebase = 'http://maps.google.com/';
			type = 'application/google-map';
			break;
