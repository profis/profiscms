	...
	function get_exec_times_summary() {
		...
		if (!empty($this->group_time_data)) {
			...
			foreach ($this->group_time_data as $key => $value) {
				$string.= $this->exec_times_wrap_begin . " $key:  {$value['total_time']} (count: {$value['count']})" . $this->exec_times_wrap_end;
			}
		}
